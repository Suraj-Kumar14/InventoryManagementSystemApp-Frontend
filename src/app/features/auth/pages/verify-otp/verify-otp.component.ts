  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 via-white to-primary-50 px-4">
      <div class="w-full max-w-md bg-white rounded-xl shadow-lg border border-neutral-200 p-8">
        <h2 class="text-2xl font-bold text-neutral-900 mb-2">Verify OTP</h2>
        <p class="text-neutral-600 mb-6">Enter the 6-digit OTP sent to your email.</p>
        ...
        <p class="text-center text-neutral-600 mt-6">
          <a routerLink="/register" class="text-primary-600 hover:text-primary-700 font-medium">Back to sign up</a>
        </p>
      </div>
    </div>
  `,
  ...
  onSubmit(): void {
    ...
    this.authService.verifyOtp(this.form.getRawValue() as { email: string; otp: string }).subscribe({
      next: (response) => {
        ...
        this.notification.success(response.message || 'User registered successfully');
        this.router.navigate(['/login']);
      },
      error: (error) => {
        this.isLoading = false;
        this.serverError = this.extractMessage(error) || 'Wrong OTP';
        this.cdr.detectChanges();
      },
    });
  }
